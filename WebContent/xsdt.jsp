...
<%
	...
	if( form1!=""&&form1 != null && form1.equals("true")) {
		...
		st.executeUpdate("insert into shownews values("+rid+",'','',now(),'',null,'学生动态','"+usertype+"')");
		response.sendRedirect("ueditor/publish.jsp?id="+rid+"&newstype=xsdt");
	}
	if(usertype.equals("admin")||usertype==""){
		rs = st.executeQuery("select * from shownews where newtype ='学生动态' order  by newstime desc");
	}else{
		rs = st.executeQuery("select * from shownews  where publishertype='"+usertype+"' and  newtype ='学生动态' ");
	}
	...
	while(rs.next()){
		...
		if(land){
			s +=("<tr><td>"+"<a href='newscontent.jsp?id="+rs.getString("id")+
					...
					rs.getString("id")+"' method=post ><input name=newstype type=hidden value=xsdt>"+
	...
	//st.executeUpdate("insert into shownews values(null,'大师不妙','你的士大夫撒旦法',now(),'sdf',null,'学生动态','学院办公室')");
	rs.close();
	st.close();
	con.close();
	//关闭连接防止异常
...
<body>
	...
	%>
		<form name="form1" method="post" action="xsdt.jsp" >
